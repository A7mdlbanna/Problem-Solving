...
int main(){       
    ...
    string countries = "aearataubebgbrcachcncocuczdeegfrgbgrhkhuidieilinitjpkrltlvmamxmyngnlnonzphplptrorsrusasesgsiskthtrtwuausveza"; 
    string country;  
    for(ll i=0; i<countries.size(); i++){
        country += toupper(countries[i]);
        if(country.size() == 2){
            cout <<"\"" <<country << "\"," << endl;
            country.clear();
        }
