...
#define st                      std::chrono::time_poll<std::chrono::system_clock> start, end; start = std::chrono::system_clock::now();
...
void srt(map<char, ll>& M)
{
    ...
    if(v.size()==0){cout << "No intersect chars";return;}
    cout << v.size() << endl;
    fra(v)cout << i << " ";
}
int main(){       
    ...
    // // }
    // // if(v.size()==0)cout << "No llersect chars";
    // // sort(all(v));
    // // cout << v.size() << endl;
    // // fra(v)cout << i << " ";
}
