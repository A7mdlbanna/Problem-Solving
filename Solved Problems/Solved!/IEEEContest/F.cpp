...
int main(){       
    ...
    sort(all(v));
    ll sum =0;
    fr(i,1,v.size())if(v[i].first.first > (v[i-1].first.first+v[i-1].first.second))sum+= v[i].first.first - (v[i-1].first.first+v[i-1].first.second);
    fra(v)cout << i.second << " ";cout << endl;
    cout << sum << endl;
}