...
ll cnt=0;ll pp;
void printSubsequences(vl v, int index, vl subarr,int n){
	  
	if(index == n){
        ll XOR = 0, XOR2 = 0;
        fr(i,0,subarr.size()){XOR^=subarr[i];}
        fr1(i,1,pp){
        bool prt = true;
        for(auto it : subarr)if(i==it)prt=false;
        if(prt){XOR2^=i;}
        }
        // cout << endl;
	    if(XOR > XOR2)cnt++;
		...
		subarr.push_back(v[index]);
		printSubsequences(v, index + 1, subarr,n);
		subarr.pop_back();
		printSubsequences(v, index + 1, subarr,n);
	}
}
