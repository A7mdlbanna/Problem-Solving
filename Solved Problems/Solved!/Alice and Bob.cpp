...
#define fraj(x)                 for (auto &j : x)
#define frak(x)                 for (auto &k : x)
...
#define sz                      size()
#define cpy(x,y)                copy(all(v), back_inserter(y, y.begin()));
#define cpy2(x,y)               copy(all(x), inserter(y, y.begin()));
...
int main(){       
    ...
    lp(n-1){
        fr(j, i+1, n){
            if(s.find(abs(v[j]-v[i])) == s.end()){
                s.insert(abs(v[j]-v[i]));
                v.pb(abs(v[j]-v[i]));
                // n++;i=0;break;
            }
            frak(s)cout << k << " ";cout << endl;
        }
        cout << endl;
    }
    // cout << s.sz;
    cout << ((s.sz)%2==1 ? "Alice" : "Bob") << endl;
}