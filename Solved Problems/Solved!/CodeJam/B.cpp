...
int main(){       
    ...
    lpk(t){
        ...
        lp(n){
            s.clear();
            lpj(p){ll x; cin >>x;s.pb(x);}sort(all(s));
            fra(s)cout << i << " ";cout << endl;

            if(i==0){pre=pos=s.back();cout << pre << endl ;continue;}
            if(abs(pre-s[0]) >= abs(pre-s.back()))pos=s[0];
            else pos=s.back();
            pre+= min(abs(pos-s[0]), abs(pos-s.back()));
            pre+= s.back()-s[0];
            cout << pos << " " <<pre << endl;
        }
