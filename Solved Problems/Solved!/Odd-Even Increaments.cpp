...
#define fr(i, x, y)             for (ll i = (ll)x; i < (ll)y; i+=2)
...
int main(){       
    ...
    w(t){
        ...
        fr(i,2,n){
            if((v[i]%2==0 and v[i-2]%2!=0) or(v[i]%2!=0 and v[i-2]%2==0)){even = false; break;}
        }
        fr(i,3,n){
            if((v[i]%2==0 and v[i-2]%2!=0) or(v[i]%2!=0 and v[i-2]%2==0)){odd = false; break;}
        }
        cout << (odd==false or even == false ? "NO" : "YES") << endl;
    }
}