...
int main(){       
    ...
    bool start = false;
    fra(s){
        // cout << i-48 << " " << digits[i-49] << endl;
        if(i-48 < digits[i-49]){i=digits[i-49]+48; start = true;}
        else if(i-48 > digits[i-49] && start)break;
    }
    cout << s;
    ...
    for (int i = 0; i < s.size(); ++i) {
        if(i-48 < v[i-49]){i=v[i-49]+48; start = true;}
        cout << max(v[s[i] - '0' - 1], s[i] - '0');
        }
}