...
int main(){       
    ...
    if(n==1){cout << v.back(); return 0;}
    sort(all(vv));
    cout << vv[0] << " "; 
    fr(i,2,n){
        ll mn=INT_MAX, ptr1 = 0, ptr2 = i-1;
        while(ptr2 < n){
            mn = min(mn, *max_element(v.begin()+ptr1, v.begin()+ptr2+1));
            // fr(j,ptr1, ptr2+1)cout << v[j] << " ";cout <<  " " << *max_element(v.begin()+ptr1, v.begin()+ptr2+1) << endl;
            ptr1++; ptr2++;
        }
            cout << mn << " ";
    }
