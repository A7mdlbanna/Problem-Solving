...
int main(){       
    ...
    // st
    deqll v(n);vl vv;
    fra(v){cin >> i.first; i.second = i.first;vv.push_back(i.first);}

    if(*max_element(all(vv)) < m){cout << n; return 0;}
    while(v.size() > 1){
    // v[0].second = v[0].first;
        if(v[0].first > m){v.emplace_back(v[0].first-m, v[0].second); v.pop_front();}
        else v.pop_front();
    // fra(v)cout << i.first << " " << i.second << endl;cout << endl;
    }
    cout << n-(find(rall(vv), v[0].second) - vv.rbegin());
    // en
}