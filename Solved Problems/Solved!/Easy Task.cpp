...
int main(){       
   ...
   w(m){
      ...
      if(a==1 )rev*=-1;
          ...
          ll b, c; cin >> b >> c;
          if(b==1 and rev == 1)dq.push_front(c);
          if(b==2 and rev == -1)dq.push_front(c);
          if(b==1 and rev == -1)dq.pb(c);
          if(b==2 and rev == 1)dq.pb(c);
      }
   } 
   if(rev==-1)reverse(all(dq));
   cout << dq.size() << endl;
   fra(dq)cout << i << " "; 
}