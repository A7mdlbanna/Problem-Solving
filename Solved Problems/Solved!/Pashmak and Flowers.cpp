...
int main(){       
    ...
    ll front ,end;
    front = count(all(v), v[0]);
    end = count(all(v), v.back());
    
    if(v.back() == v[0] and n==2)cout << "0 1";
    else if(v.back() == v[0] and n > 2)cout << "0 " << front*(end-1)/2;
    else cout << v.back()-v[0] << " " << front*end;
}