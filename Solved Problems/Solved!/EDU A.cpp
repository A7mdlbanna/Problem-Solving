...
int main(){       
    ...
    w(t){
        ...
        fr(j, 0, 100){
        fr(i,0,n-1){
            if((abs(v[i]-v[i+1]) > abs(vv[i]-v[i+1] and abs(v[i]-vv[i+1]) >= abs(v[i]-v[i+1]))) or (abs(vv[i]-vv[i+1]) > abs(v[i]-vv[i+1]) and abs(vv[i]-v[i+1]) >= abs(vv[i]-vv[i+1])))swap(v[i], vv[i]);
        }
        fra(v)cout << i << " ";cout << endl;
        fra(vv)cout << i << " ";cout << endl;
        }
        ll sum =0;
        fr(i,0,n-1){
            sum += abs(v[i]-v[i+1]) + abs(vv[i]-vv[i+1]);
        }
        cout << sum << endl;
    }
}