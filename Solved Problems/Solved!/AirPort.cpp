...
int main(){       
   fast
    ...
    ll passengers, planes, mx=0, mn=0, pos=0, r=0;
    cin >> passengers >> planes;
    vl seats(planes), seatsmx;
    fra(seats){cin >> i; seatsmx.pb(i);}
    sort(all(seats));
    sort(all(seatsmx));
    ...
    while(r<passengers){
        auto POS = find(all(seatsmx), seatsmx.back()) - seatsmx.begin();
        frr(i,planes-1,POS){if(r==passengers)break;mx += seatsmx[i]; seatsmx[i]--;r++;}
    }
    cout << mx << " " << mn;
}