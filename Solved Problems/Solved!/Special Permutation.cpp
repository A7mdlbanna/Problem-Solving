...
#define fr2(i, x, y)             for (ll i = (ll)x; i < (ll)y; i+=2)
...
int main(){       
    ...
    w(t){
        ...
        bool foundA = false;
        bool foundB = false;
        fr(i, 0, deq.size()/2){if(deq[i] < a)wrong = true; if(deq[i] == a)foundA=true;}
        fr(i, deq.size()/2, deq.size()){if(deq[i] > b)wrong = true; if(deq[i] == b)foundB=true;}
        if(wrong or !foundA or !foundB){cout << -1 << "   ";}
        fra(deq)cout << i << " "; cout << endl;
    }
}