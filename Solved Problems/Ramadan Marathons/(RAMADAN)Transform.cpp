...
int main(){
    ...
    w(t){
        ...
        ll op=0;
        fra(s){
            if(f.find(i) == string::npos){
                auto it = upper_bound(all(f), i);
                if(it!=f.end() and it!=f.begin())op+= min((*it)-i, i - *(it-1));//get the min of the litter after and before i
                else if(it!=f.end() and it==f.begin())op+= min(f[0]-i, i-'a' + 'z'-f.back()+1);// every element is greater that i
                else op+= min(i-f.back(), 'z'-i + f[0]-'a' +1);// every element is smaller than i
            }
        } 
        cout << op << endl;
    }
