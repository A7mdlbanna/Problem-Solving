...
int main(){       
    ...
    vl increase(n+1, -1), decrease(n+1, -1);
    for(int i = 1, num = 1; i <= n; i++) 
        v[i] >= v[i-1] ? increase[i] = increase[i-1] = num : num++;
    for(int i = 2, num = 1; i <= n; i++)
        v[i] <= v[i-1] ? decrease[i] = decrease[i-1] = num : num++;

    w(q){
        ll l, r;
        cin >> l >> r;
        cout << (l==r or (increase[l]==increase[r] and increase[r] != -1) or (decrease[r] == decrease[l] and decrease[r] != -1) ? "YES" : "NO") << endl;
    }
}